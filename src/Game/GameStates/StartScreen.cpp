...
StartScreen::StartScreen(const std::vector<std::string>& startScreenDescription, Game* game) 
	: m_game(game)
...
void StartScreen::processInput(std::array<bool, 349>& keys) {
	...
	if (keys[GLFW_KEY_ENTER]) {
		...
		{
		case 0:
			m_game->startNewLevel(0, Game::EGameMode::OnePlayer);
			break;
		case 1:
			m_game->startNewLevel(0, Game::EGameMode::TwoPlayers);
			break;
		default:
			break;
		}
